...
In spectromorphological terms, the process exhibits a gradual shift from gesture-based morphology (the articulated rhythmic and melodic features of the source material) toward texture-based morphology (sustained, non-directional spectral masses). What Harrison describes as "spatial density" undergoes a corresponding transformation: the initially diffuse spatial image — with sound energy distributed across multiple speaker positions — gradually collapses into regions of concentrated low-frequency resonance as higher frequencies decay and the room's modal structure asserts itself.
